// This file was automatically generated from deposit_list.coinage.soy.
...
bitex.ui.DepositList.templates.NodesProgressBar = function(opt_data) {
  ...
  output += '<div class="progress"><div class="progress-bar progress-bar-success progress-bar-striped active" role="progressbar" aria-valuenow="' + soy.$$escapeHtml(opt_data.confidence) + '" aria-valuemin="0" aria-valuemax="100" style="width:' + soy.$$escapeHtml(opt_data.confidence) + '%"><span>' + MSG_UNNAMED_155 + '</span></div></div>';
  return output;
};


bitex.ui.DepositList.templates.LabelStatus = function(opt_data) {
  return '<h5><span style="text-transform: uppercase" class="label label-' + soy.$$escapeHtml(opt_data.label) + '">' + soy.$$escapeHtml(opt_data.status) + '</span></h5>';
};
...
bitex.ui.DepositList.templates.btnShowReceipt = function(opt_data) {
  ...
  output += '<button class="btn btn-sm btn-success btn-deposit-show-receipt" data-row="' + soy.$$escapeHtml(opt_data.dataRow) + '" data-action="SHOW_RECEIPT">' + MSG_UNNAMED_157 + '<i class="glyphicon glyphicon-file"></i></button>';
  return output;
};
...
bitex.ui.DepositList.templates.btnDepositDetails = function(opt_data) {
  ...
  output += '<a class="btn btn-sm btn-info btn-deposit-view" target="_blank" href="' + soy.$$escapeHtml(opt_data.url) + '">' + MSG_UNNAMED_159 + '<i class="glyphicon glyphicon-eye-open"></i></a>';
  return output;
};
...
bitex.ui.DepositList.templates.btnShowQRCode = function(opt_data) {
  ...
  output += '<button class="btn btn-sm btn-info btn-deposit-view-qr" data-action="SHOW_QR" data-row="' + soy.$$escapeHtml(opt_data.dataRow) + '">' + MSG_UNNAMED_161 + '<i class="glyphicon glyphicon-qrcode"></i></button>';
  return output;
};
...
bitex.ui.DepositList.templates.btnInstantFiatDeposit = function(opt_data) {
  ...
  output += '<button class="btn btn-sm btn-success btn-deposit-instantaneus" data-action="INSTANTANEOUS" data-row="' + soy.$$escapeHtml(opt_data.dataRow) + '">' + MSG_UNNAMED_163 + '<i class="glyphicon glyphicon-fire"></i></button>';
  return output;
};
...
bitex.ui.DepositList.templates.btnBrokerDespositCancel = function(opt_data) {
  ...
  output += '<button class="btn btn-xs btn-danger btn-deposit-cancel" data-action="CANCEL" data-row="' + soy.$$escapeHtml(opt_data.dataRow) + '">' + MSG_UNNAMED_167 + '<i class="glyphicon glyphicon-remove-sign"></i></button>';
  return output;
};
...
bitex.ui.DepositList.templates.btnBrokerKYC = function(opt_data) {
  ...
  output += '<button class="btn btn-xs btn-warning btn-deposit-kyc" data-action="KYC" data-row="' + soy.$$escapeHtml(opt_data.userVerification) + '">' + MSG_UNNAMED_169 + '</button>';
  return output;
};
...
bitex.ui.DepositList.templates.btnBrokerDespositProgress = function(opt_data) {
  ...
  output += '<button class="btn btn-sm btn-default btn-deposit-progress" data-action="PROGRESS" data-row="' + soy.$$escapeHtml(opt_data.dataRow) + '">' + MSG_UNNAMED_171 + '<i class="glyphicon glyphicon-refresh"></i></button>';
  return output;
};
...
bitex.ui.DepositList.templates.btnBrokerDespositComplete = function(opt_data) {
  ...
  output += '<button class="btn btn-xs btn-success btn-deposit-complete" data-action="COMPLETE" data-row="' + soy.$$escapeHtml(opt_data.dataRow) + '">' + MSG_UNNAMED_173 + '<i class="glyphicon glyphicon-ok-sign"></i></button>';
  return output;
};
